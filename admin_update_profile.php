...
if(isset($_POST['update_profile'])){
    ...
    $update_profile = "UPDATE `users` SET name = '$name', email = '$email' WHERE id = '$admin_id'";
    ...
    if(!empty($image)){
       if($image_size > 2000000){
          ...
          $update_image = "UPDATE `users` SET image = '$image' WHERE id = '$admin_id'";
    ...
    if(!empty($update_pass) AND !empty($new_pass) AND !empty($confirm_pass)){
       if($update_pass != $old_pass){
          ...
          $update_pass = "UPDATE `users` SET password = '$confirm_pass' WHERE id = '$admin_id'";
